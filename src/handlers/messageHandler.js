...
const { classifyMessage, hasDashboardIntent } = require('../utils/messageClassifier');
...
function formatTransactionLine(item) {
  ...
  if (item.tipe === 'tabungan') {
    const walletSuffix = item.nama_dompet_asli ? ` (${item.nama_dompet_asli})` : '';
...
async function handleMessage(input) {
  ...
  try {
    ...
    if (isDashboardAccessCommand(originalText)) {
      ...
      const fallbackBaseUrl = process.env.WEB_APP_URL || 'http://localhost:3000';
      const fallbackLink = new URL('/dashboard', fallbackBaseUrl);
      fallbackLink.searchParams.set('whatsapp', sender);

      await message.reply(buildDashboardReply(dashboardLink || fallbackLink.toString()));
      return;
    }
    ...
    const parsedItems = (Array.isArray(parsed) ? parsed : [parsed])
      ...
      .filter((item) => item.item && Number.isFinite(item.harga) && item.harga > 0);
    ...
    for (const item of parsedItems) {
      ...
      if (!item.wallet_name && activeWallets.length === 1) {
        item.wallet_name = activeWallets[0].nama_dompet || activeWallets[0].name;
        item.nama_dompet_asli = item.wallet_name;
      }

      if (item.wallet_name) {
        const normalizedRequestedWallet = String(item.wallet_name).trim().toLowerCase();
        ...
        if (matchedWallet) {
          ...
          item.nama_dompet_asli = matchedWallet.nama_dompet || matchedWallet.name;
        }
      }
    }
      ...
      for (const row of payload) {
        try {
          ...
          if (
            ...
            String(err?.message || '').toLowerCase().includes('sudah ada')
          ) {
            skippedCount += 1;
            continue;
          }
